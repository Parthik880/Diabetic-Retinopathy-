...
def process_channel(probabilities, code, config):
    ...
    h, w = p.shape
    labels, count = ndimage.label(p >= config.pixel_threshold, structure=np.ones((3, 3)))
...
def process_maps(maps, config):
    classes={code:process_channel(p,code,config) for code,p in maps.items()}
