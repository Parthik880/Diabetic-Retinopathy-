...
class PipelineState(StrEnum):
    ...
    LESION_INFERENCE = "LESION_INFERENCE"
    LESION_MASK_PROCESSING = "LESION_MASK_PROCESSING"
    LESION_REGION_EXTRACTION = "LESION_REGION_EXTRACTION"
    LESION_RESULTS_SAVING = "LESION_RESULTS_SAVING"
    ...
    try:
        ...
        with registry.lock:
            ...
            if normalized_quality == "RECAPTURE":
                ...
                lesion_forward_count = 0

                def lesion_stage(state: str) -> None:
                    transition(PipelineState(state))

                result["lesions"] = lesions.predict(
                    analysis_path,
                    registry.models["lesion"],
                    destination / "lesions",
                    on_stage=lesion_stage,
                )
                lesion_forward_count = int(result["lesions"].get("forward_count") or 0)
                LOGGER.info("[Lesion] %s forward count = %d", eye_label, lesion_forward_count)
