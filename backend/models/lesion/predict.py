...
from runtime_device import model_parameter_device
...
def _load_original_rgb(path: Path) -> np.ndarray:
    with Image.open(path) as image:
...
def predict_lesions(
    ...
    original_rgb = _load_original_rgb(path)
    ...
    model_device = model_parameter_device(model, "Lesion")
    model.eval()
    ...
    try:
        ...
        original_probabilities = F.interpolate(
            probabilities,
        ...
        if gradcam is not None:
            for lesion, channel in LESION_CLASSES.items():
                ...
                lesion_metadata[lesion]["gradcam_path"] = str(
                    gradcam_path.resolve()
                )
    finally:
        if gradcam is not None:
            gradcam.close()
    ...
    result = {
        ...
        "bounding_box_image_path": str(boxes_path.resolve()),
        "center_point_image_path": str(centers_path.resolve()),
