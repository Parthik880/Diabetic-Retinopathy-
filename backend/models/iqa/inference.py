...
class EfficientNetIQAService:
    ...
    def __init__(
        ...
        *,
        ...
        # Same frozen pretrained backbone as upstream, bundled during setup.
        # Never fetch weights during application startup or inference.
        import os
        backbone_path = Path(os.environ['TORCH_HOME']) / 'hub' / 'checkpoints' / 'efficientnet_b0_rwightman-7f5810bc.pth'
        if not backbone_path.is_file():
            raise FileNotFoundError(f'IQA backbone is missing: {backbone_path}. Run scripts/setup.ps1.')
        efficientnet = models.efficientnet_b0(weights=None)
        efficientnet.load_state_dict(torch.load(backbone_path, map_location='cpu', weights_only=True), strict=True)
        efficientnet.eval()
