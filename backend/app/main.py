...
async def lifespan(app):
    ...
    history_path = Path(
        os.environ.get('RETINA_HISTORY_PATH', RUNTIME_ROOT / 'history' / 'index.json')
    ).expanduser().resolve()
    app.state.history = HistoryStore(history_path)
    app.state.batches = BatchAnalysisManager(
...
async def create_analysis_job(file: UploadFile = File(...), eye: Literal['OS', 'OD'] | None = Form(None)):
    ...
    image.save(path)
    jobs = app.state.jobs
    jobs.create(run_id, eye, (time.perf_counter_ns() - request_started) / 1_000_000)
    jobs.submit(
        run_id,
...
def get_analysis_job(job_id: str):
    ...
    preview = json.dumps(job, separators=(',', ':'), allow_nan=False)
    job['timing_ms']['response_serialization_ms'] = (time.perf_counter_ns() - started) / 1_000_000
    job['timing_ms']['response_payload_bytes'] = len(preview.encode('utf-8'))
    return Response(
        content=json.dumps(job, separators=(',', ':'), allow_nan=False),
...
@app.post('/api/batches/discover')
async def discover_batch(payload: dict):
    try:
        return await run_in_threadpool(app.state.batches.discover, Path(str(payload.get('input_path') or '')))
    except ValueError as exc:
        raise HTTPException(400, str(exc)) from exc


@app.post('/api/batches/{batch_id}/start', status_code=202)
async def start_batch(batch_id: str, payload: dict):
    if not re.fullmatch(r'[0-9a-f]{32}', batch_id):
        raise HTTPException(404, 'Batch not found.')
    try:
        return await run_in_threadpool(
            app.state.batches.start,
            batch_id,
            Path(str(payload.get('output_path') or '')),
            bool(payload.get('create_patient_folders', True)),
            payload.get('selections'),
            bool(payload.get('skip_unresolved', True)),
        )
...
def get_batch(batch_id: str):
    if not re.fullmatch(r'[0-9a-f]{32}', batch_id):
        raise HTTPException(404, 'Batch not found.')
    value = app.state.batches.get(batch_id)
    if value is None:
        raise HTTPException(404, 'Batch not found.')
    return value
...
def control_batch(batch_id: str, action: Literal['pause', 'resume', 'cancel']):
    if not re.fullmatch(r'[0-9a-f]{32}', batch_id):
        raise HTTPException(404, 'Batch not found.')
    try:
        return getattr(app.state.batches, action)(batch_id)
    except KeyError as exc:
        raise HTTPException(404, 'Batch not found.') from exc
