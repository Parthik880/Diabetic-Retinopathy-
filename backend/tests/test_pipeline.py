...
class PipelineRoutingTests(unittest.TestCase):
    ...
    def run_case(self, label: str):
        ...
        with (
            ...
            patch("inference.pipeline.lesions.predict", side_effect=lambda path, *_: downstream_paths.append(Path(path)) or {"lesions": {}}) as lesion,
        ):
    ...
    def test_good_uses_original_and_completes(self):
        ...
        self.assertEqual([item["state"] for item in result["state_history"]], ["WAITING", "IQA", "IQA_GOOD", "GRADING", "LESION_ANALYSIS", "PREPARING_RESULTS", "COMPLETE"])
    ...
    def test_usable_restores_once_and_sends_restored_image_downstream(self):
        ...
        self.assertEqual(result["state"], "COMPLETE")
        self.assertEqual([item["state"] for item in result["state_history"]], ["WAITING", "IQA", "IQA_USABLE", "RESTORING", "RESTORATION_COMPLETE", "GRADING", "LESION_ANALYSIS", "PREPARING_RESULTS", "COMPLETE"])
