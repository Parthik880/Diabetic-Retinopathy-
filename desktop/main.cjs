...
app.setName('RetinaGram CPU');
app.setPath('userData', path.join(app.getPath('appData'), 'RetinaGram CPU'));
...
async function start() {
  ...
  backend.once('exit', () => {
    if (!stopping && BrowserWindow.getAllWindows().length) {
      dialog.showErrorBox('Inference backend stopped', 'Close and restart RetinaGram. See the RetinaGram CPU local-data logs.');
      app.quit();
    }
  });
  let ready = false;
  for (let attempt = 0; attempt < 180; attempt++) {
    ...
    if (backend.exitCode !== null) throw new Error('Backend exited during startup. See the RetinaGram CPU local-data logs.');
    try {
  ...
  if (!ready) throw new Error('Backend startup timed out. See the RetinaGram CPU local-data logs.');
  const window = new BrowserWindow({ width: 1440, height: 960, minWidth: 800, minHeight: 650,
    title: 'RetinaGram CPU', icon: app.isPackaged
