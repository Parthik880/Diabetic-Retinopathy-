...
export function BottomNavBar({ activeTab, setActiveTab }: BottomNavBarProps) {
  return (
    <nav className="no-print fixed bottom-0 left-0 w-full z-50 flex justify-around items-center px-1 pb-safe h-16 bg-surface-container-lowest border-t-2 border-outline-variant md:hidden shadow-lg">
      {/* Capture */}
      <button
        onClick={() => setActiveTab('capture')}
        className={`flex flex-col items-center justify-center pt-2 transition-all w-1/6 ${
          activeTab === 'capture'
      ...
      <button
        ...
        className={`flex w-1/6 flex-col items-center justify-center pt-2 transition-all ${
          activeTab === 'history'
            ? 'text-primary font-bold border-t-2 border-primary -mt-[2px]'
            : 'text-on-surface-variant hover:text-primary'
        }`}
      ...
      >
        <span className="material-symbols-outlined text-[24px]" style={{ fontVariationSettings: activeTab === 'batch' ? "'FILL' 1" : "'FILL' 0" }}>stacks</span>
        <span className="font-label-lg mt-0.5 text-[10px] tracking-tight">Batch</span>
      </button>
    </nav>
  );
}
