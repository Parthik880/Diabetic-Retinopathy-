...
export function ReportScreen({ patient, onUpdatePatient, onOpenReferralModal }: ReportScreenProps) {
  ...
  const [sendingTo, setSendingTo] = useState<PatientRecord | null>(null);
  ...
  return (
    <div className="report-screen mx-auto w-full max-w-6xl px-4 py-7 pb-32 md:px-6 md:pb-14">
      ...
      <div className="no-print mt-5 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
        <button type="button" onClick={() => setSendingTo(patient)} disabled={saveState !== 'idle'} className="flex min-h-14 items-center justify-center gap-2 rounded-xl bg-primary px-6 text-base font-extrabold text-on-primary hover:bg-primary-container focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 disabled:opacity-50">
          <span className="material-symbols-outlined text-xl" aria-hidden="true">send</span>Send Report
        </button>
      ...
      {sendingTo && <SendReportModal patient={sendingTo} onClose={() => setSendingTo(null)} />}
    </div>
  );
}
