...
export const PIPELINE_STATE_LABELS: Record<AnalysisState, string> = {
  ...
  LESION_ANALYSIS: 'Preparing lesion analysis...',
  LESION_INFERENCE: 'Running lesion model on GPU...',
  LESION_MASK_PROCESSING: 'Processing lesion probability masks...',
  LESION_REGION_EXTRACTION: 'Extracting and filtering lesion regions...',
  LESION_RESULTS_SAVING: 'Saving lesion results...',
...
export const ACTIVE_ANALYSIS_STATES = new Set<AnalysisState>([
  ...
  'RESTORATION_COMPLETE', 'GRADING', 'LESION_ANALYSIS', 'LESION_INFERENCE',
  'LESION_MASK_PROCESSING', 'LESION_REGION_EXTRACTION', 'LESION_RESULTS_SAVING', 'PREPARING_RESULTS',
]);
...
export interface AnalysisResult {
  ...
  lesions: {
    ...
    timing_ms?: Record<string, number | string | boolean>;
  ...
  device_name?: string;
  transport_timing?: {
    upload_queue_roundtrip_ms: number;
    request_to_result_ms: number;
    final_poll_roundtrip_ms: number;
    result_json_bytes: number;
    polling_interval_ms: number;
    backend_receive_to_queue_ms?: number;
    queue_wait_ms?: number;
    inference_worker_ms?: number;
...
export interface BatchEyeSnapshot {
  eye: 'OS' | 'OD';
  source_name: string;
...
export interface BatchPatientSnapshot {
  index: number;
  patient_id: string;
  name: string;
  images: Array<'OS' | 'OD'>;
  status: BatchPatientStatus;
  discovery_status: 'READY' | 'NEEDS_REVIEW' | 'INVALID';
  issues: string[];
  eye_candidates: Record<'OS' | 'OD', BatchEyeCandidate[]>;
  progress: string;
  eyes: Partial<Record<'OS' | 'OD', BatchEyeSnapshot>>;
}

export interface BatchSnapshot {
  batch_id: string;
  state: 'Review' | 'Processing' | 'Pausing' | 'Paused' | 'Cancelling' | 'Cancelled' | 'Completed' | 'Failed';
  input_path: string;
  output_root: string | null;
  mode: 'structured' | 'simple' | 'mixed';
  patients: BatchPatientSnapshot[];
...
async function batchJson(response: Response, fallback: string): Promise<BatchSnapshot> {
  const value = await response.json().catch(() => null);
  if (!response.ok) throw new Error(responseError(value, fallback));
  return value as BatchSnapshot;
}

export async function discoverBatch(inputPath: string): Promise<BatchSnapshot> {
  return batchJson(await fetch('/api/batches/discover', {
...
export async function startBatch(
  ...
  selections: Record<string, Partial<Record<'OS' | 'OD', string>>>,
  skipUnresolved: boolean,
): Promise<BatchSnapshot> {
  return batchJson(await fetch(`/api/batches/${batchId}/start`, {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      output_path: outputPath,
      create_patient_folders: createPatientFolders,
      selections,
      skip_unresolved: skipUnresolved,
    }),
  }), 'Batch analysis could not be started.');
}

export async function getBatch(batchId: string): Promise<BatchSnapshot> {
  return batchJson(await fetch(`/api/batches/${batchId}`), 'Batch status could not be read.');
}

export async function controlBatch(batchId: string, action: 'pause' | 'resume' | 'cancel'): Promise<BatchSnapshot> {
  return batchJson(await fetch(`/api/batches/${batchId}/${action}`, { method: 'POST' }), `Batch ${action} failed.`);
}
...
export async function analyzeImage(
  ...
  try {
    ...
    const uploadQueueRoundtrip = performance.now() - requestStarted;
    ...
    while (true) {
      ...
      const pollRoundtrip = performance.now() - pollStarted;
      ...
      if (job.state === 'COMPLETE' || job.state === 'RECAPTURE_REQUIRED') {
        ...
        job.result.transport_timing = {
          upload_queue_roundtrip_ms: Math.round(uploadQueueRoundtrip * 1000) / 1000,
          request_to_result_ms: Math.round((performance.now() - requestStarted) * 1000) / 1000,
          final_poll_roundtrip_ms: Math.round(pollRoundtrip * 1000) / 1000,
          result_json_bytes: new Blob([JSON.stringify(job.result)]).size,
          polling_interval_ms: 250,
          backend_receive_to_queue_ms: job.timing_ms?.backend_receive_to_queue_ms ?? undefined,
          queue_wait_ms: job.timing_ms?.queue_wait_ms ?? undefined,
          inference_worker_ms: job.timing_ms?.inference_worker_ms ?? undefined,
          response_serialization_ms: job.timing_ms?.response_serialization_ms ?? undefined,
          response_payload_bytes: job.timing_ms?.response_payload_bytes ?? undefined,
        };
        console.info('[RetinaGram][Transport Timing]', job.result.transport_timing);
        return job.result;
      }
