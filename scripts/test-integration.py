...
with patch.object(socket.socket, 'connect', offline_connect):
    with TestClient(app) as client:
        ...
        queued = client.post('/api/analysis-jobs', files={'file': ('retina.jpg', data, 'image/jpeg')}, data={'eye': 'OS'})
        ...
        assert [item['state'] for item in result['state_history']] == ['WAITING', 'IQA', 'GRADING', 'LESION_ANALYSIS', 'COMPLETE']
        ...
        exported = client.post('/api/reports/export', json={
            ...
            'patient': {'name': 'GPU Test', 'id': 'LOCAL-TEST', 'age': 1, 'gender': 'Other', 'eye': 'OD',
                        'scan_datetime': '2026-09-05T20:00:00+05:30'},
        })
        assert exported.status_code == 200, exported.text
        report_folder = Path(exported.json()['folder'])
        ...
        record = {'health': health, 'result': result, 'restoration': restored.json(), 'report_folder': str(report_folder),
        ...
        print(json.dumps({'quality': result['quality'], 'grading': result['grading'],
